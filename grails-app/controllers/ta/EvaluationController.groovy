...
class EvaluationController {
	...
	def consult() {
		def auxList = Evaluation.list()
		def evaluationList = auxList.findAll {
			if(it.applicationDate.getDateString() == params.getDateString()){
				print "s"
			} 
			print "n"
		}
		if (evaluationList == null) {
			...
			render view: "SearchpropertybyDate", model: [evaluationInstanceList: [], evaluationInstanceCount: 0]
		} else {
			render view: "SearchpropertybyDate", model: [evaluationInstanceList: evaluationList, evaluationInstanceCount: evaluationList.size()]
		}
	}
	
	def SearchpropertybyDate(){
		render view: "SearchpropertybyDate"
	}
}
